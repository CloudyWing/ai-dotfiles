...
`model_reasoning_effort` 不影響模型單價。設為 `low` 只縮短推理長度，不會使昂貴模型變便宜；控制成本要從模型選用與讀取量著手，不是調降 effort。

降低成本的方式是減少執行端自行探索的讀取量，不是縮小任務範圍。
...
- 控制 subagent 的模型繼承。Codex 的 subagent 預設繼承父 Agent 的模型與推理強度，因此以昂貴模型派工時，每個 subagent 都以同一模型執行，token 隨並行數累加。在該檔位的設定檔加入 `[agents]` 的 `default_subagent_model` 與 `default_subagent_reasoning_effort`，把讀取類工作降到成本較低的模型，主 Agent 保留高階模型做判斷。
- 未設定 `max_concurrent_threads_per_session` 時使用 Codex 預設並行數，該並行數直接乘上每個 subagent 的消耗。需要上限時於設定檔明列。
- 仍需限制讀取量時，另在 prompt 明文要求單一 agent 執行並說明理由。此為補充手段，不能取代設定檔的模型降級。
- 額度不足以支撐完整任務時，在 prompt 加入降級指示：要求先寫出已確認的結果並註明實際覆蓋範圍，不要在零產出的情況下中斷。實測該指示可使中斷的派遣仍交付部分成果。

subagent 已降級為低成本模型時，冷啟動的消耗結構隨之改變，前述冷啟動門檻應依降級後的實測值重新校準，不沿用降級前的數據。
