...
`deep` 的門檻明顯較高，理由是其單次消耗量級不同。實測 `gpt-6-astra` 在約 30 分鐘、9 次命令執行內將 `primary` 用量由 0% 推升至 96%，該次派遣尚未產出任何結論即因額度耗盡而以 `turn.failed` 終止。剩餘額度低於 90% 時，即使高於預設檔位的 30% 門檻也不啟動 `deep`，改用預設檔位或等待視窗重設。
...
`deep` 一律經使用者確認後才使用，不自動升級。理由是 `deep` 單次派工可能耗盡整個 `primary` 視窗，等同用掉該視窗全部派工餘裕，這個取捨屬於使用者的資源分配決定。確認請求必須明白告知本次派遣可能耗盡當前視窗，不只列出剩餘百分比。
