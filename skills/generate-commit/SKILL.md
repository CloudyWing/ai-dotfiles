...
4. 對所有 staged 檔案逐一執行 `git diff --cached [file]` 取得具體的程式碼差異，不得因「檔名已能說明意圖」而略過，以確保 Subject 與 Body 的描述 100% 來自 diff 實際內容。

🚨 **絕對禁止**根據對話上下文或記憶**推斷或描述變更內容**。Subject 與 Body 中對「改了什麼」的描述，必須 100% 來自 `git diff` 的實際輸出，不得涵蓋 diff 以外的任何變更。

對話上下文僅可用於補充 **Body 的動機說明**（解釋「為什麼改」），且必須與當次 diff 有直接關聯。若無把握，略去動機說明即可。
...
1. 若所有變更圍繞同一個功能或修正 → **單一 commit**，直接進入步驟 4。
2. 若變更涵蓋兩個以上不相關的語意，先判斷是否有跨語意群組的單一檔案：
   - **無跨組檔案（純檔案級拆分）**：輸出拆分計畫後直接依序執行（見步驟 5）。
   - **有跨組檔案**：判斷各區塊是否可獨立運作（拆開後不會導致編譯失敗或功能異常）：
     - **可獨立**（hunk 級拆分）：輸出拆分計畫，標注每個 commit 涉及的行範圍，並列出需執行 `git add -p` 的檔案，等待使用者自行完成 staging 後回報，再執行後續 commit。
     - **不可獨立**：合併為單一 commit，在標題涵蓋主要變更。

若使用者選擇不拆分，合併為單一 commit。

**拆分計畫輸出格式（檔案級與 hunk 級共用）：**
...
Commit 2：<commit message>
  - file_b.cs（第 80-120 行）
  - file_c.md

⚠️ file_b.cs 含跨 commit 的變更，請執行 `git add -p file_b.cs` 完成 staging 後回報。
```

檔案級拆分無行範圍，直接省略行範圍標注與 `git add -p` 提示。

### 4. 執行 Commit

依據下方規範產生 commit 訊息後直接執行，不需要詢問確認：
