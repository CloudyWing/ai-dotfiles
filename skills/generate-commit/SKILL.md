...
- 拆分 commit 的最小單位是**檔案**，無法對同一檔案內的不同區塊分別 staging（`git add -p` 為互動式指令，無法在自動化環境執行）。
- 因此，拆分的前提是：**每個檔案只歸屬於一個語意群組**，不存在跨群組的檔案。
...
2. 若變更涵蓋兩個以上不相關的語意（例如：A 組是新增功能、B 組是修正錯字）：
   ...
   - **無跨組檔案**：建議拆分，每組各自 commit。
   - **有跨組檔案**：該檔案會迫使相關群組合併。若合併後仍有可獨立的群組，維持部分拆分；若所有群組因此連鎖合併，建議**單一 commit** 並在標題涵蓋主要變更。
