...
使用者確認後，Clarify 決定摘要內容與文字表達，將摘要原文交由資源派遣的 Codex 寫入 `<work-root>/.local/ai-sessions/handoff/requirement-summary.md`，並套用 `check-markdown`。欄位與上列一致，措辭保留當初的具體用語。Clarify 不直接執行寫檔與格式校對。此檔為 AI-facing 交接檔，屬 `handoff/` 結案自動清理的例外，與 `design.md` 同樣保留。

寫檔時機限於使用者確認之後。確認前的摘要仍是討論中的草稿，落檔會讓未定案內容取得交接檔的地位。

覆寫前備份由資源派遣的 Codex 執行。若目標檔案已存在，先將既有檔案改名為 `<原檔名>.<yyyyMMdd_HHmmss>` 移入 `<work-root>/.local/ai-sessions/history/`（目錄不存在時自動建立），再寫入新內容。
...
2. **逐項核對驗收檢查清單**（見下節）。
3. 若有任一項不合格，先判定缺漏類型。若缺漏清單全部是可機械對照的項目，且已指明修改位置與目標內容，例如同步計數、刪除段落、改寫編號或補上明文句，改以資源派遣交由 Codex 執行。若缺漏涉及重新設計或需求重新解讀，才以 Agent 工具**再次派生 Design sub-agent**，將具體缺漏清單作為輸入傳入（例如「§9 Phase 3 缺少 [REWRITE] 對應的移除清單」），計為新一輪。
...
- **嚴禁修改任何程式碼或專案檔案（Crucial）**：整個釐清過程不得新增、修改或刪除任何檔案。唯一的常態寫入是使用者確認後由資源派遣的 Codex 執行的 `<work-root>/.local/ai-sessions/handoff/requirement-summary.md`；摘要內容與文字表達由 Clarify 決定，需求摘要另透過 Agent 工具傳遞給 Design。
