...
- **程式面項目**：已觀察的問題、改善方向。
- **功能面項目**：新增功能、擴充方向。
- 上兩欄寫入需求摘要檔時，各自使用 `## 程式面項目` 與 `## 功能面項目` 二級標題，節內以 `| # | 項目 | 內容 |` 表格逐項列出，`#` 欄為純數字編號，兩節合併後不得重複。兩個標題在檔案中各出現恰好一次，節內只能是這一個表格，表格前後不得有說明段落、清單或其他表格；需要補充說明的內容寫在「需求背景」或其他欄位。沒有項目的一節保留標題，節內只寫一行「無」。`Architect` 的 `〔需求 #n〕` 標註、Developer 的「需求對照」節與 `Collect` 的結構核對都以此編號為鍵，格式不符時 workflow 回收會被擋下。
- **排除項目**（若有）：明確不做的範圍。
