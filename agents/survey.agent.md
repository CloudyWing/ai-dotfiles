...
1. 掃描專案根目錄，辨識技術棧（語言、框架、套件管理工具、建置工具、CI/CD 設定）。掃描過程中主動偵測以下非顯而易見的機制，發現時記錄供 Phase 2 優先納入：
   - 自訂基底類別（abstract base class）或對標準函式庫、第三方套件的自訂封裝。
   - 非典型的連線、執行緒或非同步生命週期管控（如 `ThreadLocal`、`ThreadStatic`、`AsyncLocal`）。
   - 影響執行期行為的自訂 Attribute 或慣例。
   - 非標準的 SQL 撰寫慣例或資料格式規則。
...
| 基礎設施機制 | `infrastructure-patterns.md` | 自訂封裝層、連線與執行緒管控策略、設定讀取方式、非標準慣例 |
| 決策與債務 | `decisions-and-debt.md` | 架構決策紀錄（ADR）、已知技術債、使用陷阱（偏離一般慣例且容易踩雷的寫法） |
