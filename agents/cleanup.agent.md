...
- 以下情況不自行刪除，輸出至 `cleanup-review.md` 供使用者確認後再處理：
  - 完整註解掉的程式碼。
...
- 時間型別遵循**專案現有慣例**：若專案已統一使用 `DateTime`，則維持；若已統一使用 `DateTimeOffset`，則維持。**不自動轉換 `DateTime` ↔ `DateTimeOffset`**（屬於破壞性變更）。若發現 `DateTime.Kind` 在同一專案內混用（`Local`/`Utc`/`Unspecified`），輸出至 `cleanup-review.md` 供使用者確認。
...
- 發現直接 `new HttpClient()` 的情況，輸出至 `cleanup-review.md` 並說明建議改用 `IHttpClientFactory` 的理由（不自動修改）。
...
6. **確認項目集中輸出**：無法自行判斷、需使用者確認才能處理的項目，統一輸出至 `.local/ai-sessions/cleanup-review.md`（目錄不存在時自動建立），不得散落於對話或以行內註解標記。每個項目格式：相對路徑、行號、程式碼片段（前後各 2 行）、標記原因。
